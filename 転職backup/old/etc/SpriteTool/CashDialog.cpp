// CashDialog.cpp : インプリメンテーション ファイル
//
...
CCashDialog::CCashDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CCashDialog::IDD, pParent)
{
	//{{AFX_DATA_INIT(CCashDialog)
	m_Edit_Caption = _T("");
	//}}AFX_DATA_INIT
}


void CCashDialog::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCashDialog)
...
BEGIN_MESSAGE_MAP(CCashDialog, CDialog)
	//{{AFX_MSG_MAP(CCashDialog)
...
/////////////////////////////////////////////////////////////////////////////
// CCashDialog メッセージ ハンドラ

void CCashDialog::OnOK() 
{
...
BOOL CCashDialog::PreTranslateMessage(MSG* pMsg) 
{
		...
		case WM_KEYUP:														// キーアップだったら
			...
			case VK_DELETE:													// DELETEキーなら処理しましょう
				...
				if(delete_flg == ON){										// デリート許可は下りてますか？
					if(	pApp->CashDialog.m_List_Cash.GetCurSel() != 0){		// キャッシュのカーソルは1段目？
						work_cnt = pApp->CashDialog.m_List_Cash.GetCurSel();// 違うなら、今のカーソル位置を取得
						...
						pApp->CashDialog.m_List_Cash.DeleteString(work_cnt);
						...
						for(i = work_cnt-1;i < pApp->cash_cnt;i++){
							...
							if((pApp->cash_data[i].left == pApp->cash_data[i].right) || (pApp->cash_data[i].top == pApp->cash_data[i].bottom)){
								...
								pApp->CashDialog.m_List_Cash.InsertString(i,work);			// 消した位置に注釈つきでキャッシュ書き直し
								pApp->CashDialog.m_List_Cash.SetCurSel(i);					// で、その位置にカーソル戻そう
							}
						}
						...
						pApp->CashDialog.m_List_Cash.SetCurSel(0);			// 一応、1番目にカーソル戻しとこう
			...
			case VK_RETURN:													// RETURNキーなら
				if(enter_flg == ON){										
					...
					if(	pApp->CashDialog.m_List_Cash.GetCurSel() != 0){						// カーソル位置0番目じゃない？
						work_cnt = pApp->CashDialog.m_List_Cash.GetCurSel();				// じゃあ、カーソル位置記憶して
						pApp->cash_string[work_cnt - 1] = m_Edit_Caption;					// 文章保存用の変数に今の文章渡そう
						pApp->CashDialog.m_List_Cash.DeleteString(work_cnt);				// で、その位置のキャッシュ消しましょう
						...
						pApp->CashDialog.m_List_Cash.InsertString(work_cnt+1,work);			// 消した位置に注釈つきでキャッシュ書き直し
						pApp->CashDialog.m_List_Cash.SetCurSel(work_cnt+1);					// で、その位置にカーソル戻そう
						enter_flg = OFF;	
					}
...
void CCashDialog::OnDblclkListCash() 
{
...
void CCashDialog::DrawExec()
{
	Invalidate(false);
}
void CCashDialog::DrawExecEx()
{
	CSpriteToolApp* pApp = (CSpriteToolApp*)AfxGetApp();
	ASSERT_VALID(pApp);
	...
	work = pApp->CashDialog.m_List_Cash.GetCurSel();						// カーソル位置保存
...
void CCashDialog::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void CCashDialog::OnSelchangeListCash() 
{
	// TODO: この位置にコントロール通知ハンドラ用のコードを追加してください
	CSpriteToolApp* pApp = (CSpriteToolApp*)AfxGetApp();
	ASSERT_VALID(pApp);
