...
CBillbord::SetModelData(HWND hwnd,LPDIRECT3DDEVICE8 p_Device,char** texture_name){
	D3DCOLOR color = D3DCOLOR_ARGB(0,0,0,255);
	...
	D3DXCreateTextureFromFileExA(p_Device, texture_name[0],0,0,0,NULL,D3DFMT_UNKNOWN,D3DPOOL_DEFAULT,
		D3DX_DEFAULT,D3DX_DEFAULT,color,&ita,NULL,&texture);
	p_Device->CreateVertexBuffer(4*sizeof(VERTEX),0,FVF_VERTEX,D3DPOOL_DEFAULT,&vertex_buffer);
...
CBillbord::SetVbuffer(LPDIRECT3DDEVICE8 p_Device){
	...
	pVertex->color = D3DCOLOR_ARGB((int)(alpha*255), 0xff, 0xff, 0xff);
	pVertex++;
...
CBillbord::Draw(LPDIRECT3DDEVICE8 p_Device){
	...
	if(draw_flg == 1){
		...
		if(pApp->EditDialog.m_Check_Add_Alpha.GetCheck() == (int)false){
		...
		if(pApp->EditDialog.m_Check_Alpha.GetCheck() == (int)true){
			p_Device->SetTextureStageState(0,D3DTSS_ALPHAOP,D3DTOP_MODULATE);	// テクスチャのアルファを適用
		}
		else{
			p_Device->SetTextureStageState(0,D3DTSS_ALPHAOP,D3DTOP_DISABLE);	// テクスチャのアルファを無効に
		}

		p_Device->SetTextureStageState(0,D3DTSS_ALPHAARG1,D3DTA_TEXTURE);	// 頂点の色を使用
		p_Device->SetTextureStageState(0,D3DTSS_ALPHAARG2,D3DTA_DIFFUSE);	// レンダリング時の色の計算方法の設定


		// ビルボード用行列代入
#if 0
		matrix._11 = pApp->direct_x.camera.matView._11*scale;
		matrix._12 = pApp->direct_x.camera.matView._21;
		matrix._13 = pApp->direct_x.camera.matView._31;
		matrix._21 = pApp->direct_x.camera.matView._12;
		matrix._22 = pApp->direct_x.camera.matView._22*scale;
		matrix._23 = pApp->direct_x.camera.matView._32;
		matrix._31 = pApp->direct_x.camera.matView._13;
		matrix._32 = pApp->direct_x.camera.matView._23;
		matrix._33 = pApp->direct_x.camera.matView._33*scale;
#else 
		matrix._11 = pApp->direct_x.camera.matView._11;
		matrix._12 = pApp->direct_x.camera.matView._21;
		matrix._13 = pApp->direct_x.camera.matView._31;
		matrix._21 = pApp->direct_x.camera.matView._12;
		matrix._22 = pApp->direct_x.camera.matView._22;
		matrix._23 = pApp->direct_x.camera.matView._32;
		matrix._31 = pApp->direct_x.camera.matView._13;
		matrix._32 = pApp->direct_x.camera.matView._23;
		matrix._33 = pApp->direct_x.camera.matView._33;

		D3DXMatrixScaling(&matScale,scale*sclx,scale*scly,scale*sclz);
		D3DXMatrixScaling(&matScale2,sclx,scly,sclz);

		matrix *= matScale ;//* matScale2;
#endif
...
CBillbord::SetAlpha(float palpha)
{
	alpha = palpha;
}
CBillbord::SetScaleXYZ(float x,float y,float z)
{
	sclx = x;
	scly = y;
	sclz = z;
