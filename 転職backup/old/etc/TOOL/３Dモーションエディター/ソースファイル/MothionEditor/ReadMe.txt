========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : MothionEditor
========================================================================


AppWizard が生成した MothionEditor には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには MothionEditor を構成している各ファイルの概要説明が含まれています。
...
MothionEditor.h
    ...
    みます） をインクルードし、また CMothionEditorApp アプリケーションクラスを宣
    言します。

MothionEditor.cpp
    このファイルは CMothionEditorApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\MothionEditor.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル MothionEditor.rc によってインクルード
    されます。

res\MothionEditor.rc2
...
MothionEditorDoc.h, MothionEditorDoc.cpp - ドキュメント
    これらのファイルは CMothionEditorDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CMothionEditorDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

MothionEditorView.h, MothionEditorView.cpp - ビュー
    これらのファイルは CMothionEditorView クラスを含みます。
    CMothionEditorView オブジェクトは CMothionEditorDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) MothionEditor.pch 
