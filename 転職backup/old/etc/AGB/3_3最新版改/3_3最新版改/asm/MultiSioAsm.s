...
@********************************************************************
    ...
    .INCLUDE    "asm/MultiSioDefine.s"


    .ifdef MULTI_SIO_DI_FUNC_FAST

@--------------------------------------------------------------------
@-              受信データ・チェックバッファ更新 ルーチン           -
@--------------------------------------------------------------------
...
MultiSioRecvBufChange:
        ...
        @ *(vu8 *)REG_IME = 0   割込不許可（29クロック）

        strb    r8,  [r12, #OFFSET_REG_IME]

        @ Ms.RecvCheckBufp[i] ←→ Ms.LastRecvBufp[i]      受信バッファ更新
...
MultiSioIntr:
        ...
        @     REG_SIOCNT->Data = MULTI_SIO_SYNC_DATA          同期データ セット

        strh    r9, [r12, #REG_SIOMLT_SEND - REG_SIOMLT_RECV]

        @     Ms.CurrentSendBufp ←→ Ms.NextSendBufp         送信バッファ変更
...
3:
        ...
        @ 受信データ処理（最大約160クロック／ウェイト期間に含まれる）
...
6:
        ...
        @     Ms.CurrentRecvBufp[i] ←→ Ms.LastRecvBufp[i]   受信バッファ変更
