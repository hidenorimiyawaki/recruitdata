//=================================================================================================
//    [ MultiScreen ] DirectX Graphics 3D sample
//    マルチスクリーンサンプル
//
//    操作
//    　「Ｐ」			ポーズＯＮ・ＯＦＦ
...
//-------- 定数定義
#define CLASS_NAME		"MultiScreen"									// ウィンドウクラス名
#define CAPTION_NAME	"[ MultiScreen ] DirectX Sample Program"		// キャプション名

const int	PARTICLE_MAX		= 200;									// パーティクル最大数
const int	PARTICLE_INIT		= 50;									// パーティクル初期数
...
D3DXVECTOR3				g_vCameraRotation;			// カメラ自動回転用ベクトル
D3DXVECTOR3				g_vCameraRotSpeed;			// カメラ自動回転速度用ベクトル
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int iCmdShow)
{
	...
	g_vCameraRotation = D3DXVECTOR3(0, 0, 0);
	g_vCameraRotSpeed = D3DXVECTOR3(0.008f, 0.007f, 0.006f);
...
HRESULT Initialize3DEnvironment(HWND hWnd, bool bWindow)
{
	...
	// 表示領域サイズの設定
	g_D3DPresentParam.BackBufferWidth  = SCREEN_WIDTH  / 2;		// マルチスクリーンなので小さくてＯＫ
	g_D3DPresentParam.BackBufferHeight = SCREEN_HEIGHT / 2;
...
HRESULT Render3DEnvironment()
{
	...
	static RECT	SrcRect = {  0,   0, 320, 240};		// 転送元領域
	static RECT	DstRect[] = {			// 転送先領域
		{  0,   0, 320, 240},			// スクリーン０（左上）
		{320,   0, 640, 240},			// スクリーン１（右上）
		{  0, 240, 320, 480},			// スクリーン２（左下）
		{320, 240, 640, 480},			// スクリーン３（右下）
	};
	...
	if(SUCCEEDED(g_pD3DDevice->BeginScene())) {		// シーン開始
		...
		// 画面は４つ
		for(nScreen = 0; nScreen < 4; nScreen++) {
			switch(nScreen) {		// スクリーンごとに変化をつける
			case 0:
				g_pD3DDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);	// 面で描画
				// キーによる視点位置の移動など
			...
			case 1:
				// ビューマトリックス設定
				xx = (float)cos(g_fRotation / 4) * 600.0f;			// 回転速度を１／４に
				zz = (float)sin(g_fRotation / 4) * 600.0f;
				D3DXMatrixLookAtLH(&g_ViewMatrix, &D3DXVECTOR3(xx, g_fHeight, zz), &D3DXVECTOR3(0, 0, 0), &D3DXVECTOR3(0, 1, 0));
				g_pD3DDevice->SetTransform(D3DTS_VIEW, &g_ViewMatrix);	// ビューマトリックスをセット
				break;
			case 2:
				// ビューマトリックス設定
				xx = (float)cos(g_vCameraRotation.x) * 600.0f;		// 自動で回転
				yy = (float)sin(g_vCameraRotation.y) * 300.0f + 400.0f;
				zz = (float)sin(g_vCameraRotation.z) * 600.0f;
				g_vCameraRotation += g_vCameraRotSpeed;
				D3DXMatrixLookAtLH(&g_ViewMatrix, &D3DXVECTOR3(xx, yy, zz), &D3DXVECTOR3(0, 0, 0), &D3DXVECTOR3(0, 1, 0));
				g_pD3DDevice->SetTransform(D3DTS_VIEW, &g_ViewMatrix);	// ビューマトリックスをセット
				break;
			case 3:
				g_pD3DDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);	// ワイヤーフレームで描画
				break;
			}
			...
			switch(nScreen) {
			case 0:
				sprintf(str, "[ SCREEN 0 ] Normal Rotation\n");			strcat(g_DebugStr, str);
				...
				break;
			case 1:
				sprintf(str, "[ SCREEN 1 ] Slow Rotation\n");			strcat(g_DebugStr, str);
				break;
			case 2:
				sprintf(str, "[ SCREEN 2 ] Auto Rotation\n");			strcat(g_DebugStr, str);
				break;
			case 3:
				sprintf(str, "[ SCREEN 3 ] Wire Frame Rendering\n");	strcat(g_DebugStr, str);
				break;
			}
			g_pD3DXFont->DrawText(g_DebugStr, -1, &strRect, DT_LEFT, D3DCOLOR_ARGB(0xff, 0xff, 0xff, 0xff));

			// バックバッファをプライマリバッファにコピー
			if(FAILED(g_pD3DDevice->Present(&SrcRect, &DstRect[nScreen], NULL, NULL))) g_pD3DDevice->Reset(&g_D3DPresentParam);
		}
