========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : VectorSetTool
========================================================================


AppWizard が生成した VectorSetTool には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには VectorSetTool を構成している各ファイルの概要説明が含まれています。
...
VectorSetTool.h
    ...
    みます） をインクルードし、また CVectorSetToolApp アプリケーションクラスを宣
    言します。

VectorSetTool.cpp
    このファイルは CVectorSetToolApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\VectorSetTool.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル VectorSetTool.rc によってインクルード
    されます。

res\VectorSetTool.rc2
...
VectorSetToolDoc.h, VectorSetToolDoc.cpp - ドキュメント
    これらのファイルは CVectorSetToolDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CVectorSetToolDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

VectorSetToolView.h, VectorSetToolView.cpp - ビュー
    これらのファイルは CVectorSetToolView クラスを含みます。
    CVectorSetToolView オブジェクトは CVectorSetToolDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) VectorSetTool.pch 
